...
if (!in_array($activeClassId, $myClassIds)) $activeClassId = $myClassIds[0];

// ── Helper: confirm student is in one of teacher's classes ─
function verifyStudentInClass(PDO $db, int $studentUserId, array $classIds): bool {
    if (empty($classIds)) return false;
    $in = implode(',', array_fill(0, count($classIds), '?'));
    $st = $db->prepare(
        "SELECT 1 FROM students s
         JOIN users u ON u.id = s.user_id
         WHERE s.user_id = ? AND s.class_id IN ($in) AND u.status = 'active'"
    );
    $st->execute(array_merge([$studentUserId], $classIds));
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($action === 'save_marks') {
        ...
        if ($examMarks === null) { echo json_encode(['success'=>false,'message'=>'Exam marks are required.']); exit; }
        if ($examMarks < 0 || $examMarks > $totalExamMarks) { echo json_encode(['success'=>false,'message'=>"Exam marks must be 0–{$totalExamMarks}."]); exit; }
        if ($reexamMarks !== null && ($reexamMarks < 0 || $reexamMarks > $totalExamMarks)) { echo json_encode(['success'=>false,'message'=>"Re-exam marks must be 0–{$totalExamMarks}."]); exit; }
        if (!verifyStudentInClass($db, $studentUserId, $myClassIds)) { echo json_encode(['success'=>false,'message'=>'Student not found in your classes.']); exit; }
    ...
    if ($action === 'get_exam_scores') {
        ...
        if (!verifyStudentInClass($db, $uid, $myClassIds)) { echo json_encode(['success'=>false,'message'=>'Student not found.']); exit; }
        $st = $db->prepare('SELECT id, exam_name AS score_name, marks_obtained, total_marks, exam_date AS score_date FROM exam_scores WHERE student_user_id=? ORDER BY exam_date DESC, created_at DESC');
        $st->execute([$uid]);
    ...
    if ($action === 'add_exam_score') {
        ...
        if (!verifyStudentInClass($db, $uid, $myClassIds)) { echo json_encode(['success'=>false,'message'=>'Student not found.']); exit; }
        if ($name === '') { echo json_encode(['success'=>false,'message'=>'Exam name is required.']); exit; }
        if ($marks === '' || !is_numeric($marks)) { echo json_encode(['success'=>false,'message'=>'Marks obtained are required.']); exit; }
        $marks = (float)$marks;
        if ($marks < 0 || $marks > $total) { echo json_encode(['success'=>false,'message'=>"Marks must be 0–{$total}."]); exit; }

        $db->prepare('INSERT INTO exam_scores (student_user_id, teacher_id, exam_name, marks_obtained, total_marks, exam_date) VALUES (?,?,?,?,?,?)')
    ...
    if ($action === 'delete_exam_score') {
        ...
        echo json_encode($st->rowCount() > 0 ? ['success'=>true,'message'=>'Score deleted.'] : ['success'=>false,'message'=>'Not found or permission denied.']); exit;
    }
    ...
    if ($action === 'get_test_scores') {
        $uid = (int)($_POST['student_user_id'] ?? 0);
        if (!verifyStudentInClass($db, $uid, $myClassIds)) { echo json_encode(['success'=>false,'message'=>'Student not found.']); exit; }
        $st = $db->prepare('SELECT id, test_name AS score_name, marks_obtained, total_marks, test_date AS score_date FROM test_scores WHERE student_user_id=? ORDER BY test_date DESC, created_at DESC');
        $st->execute([$uid]);
        echo json_encode(['success'=>true,'scores'=>$st->fetchAll(PDO::FETCH_ASSOC)]); exit;
    }
    ...
    if ($action === 'add_test_score') {
        ...
        if (!verifyStudentInClass($db, $uid, $myClassIds)) { echo json_encode(['success'=>false,'message'=>'Student not found.']); exit; }
        if ($name === '') { echo json_encode(['success'=>false,'message'=>'Test name is required.']); exit; }
        if ($marks === '' || !is_numeric($marks)) { echo json_encode(['success'=>false,'message'=>'Marks obtained are required.']); exit; }
        $marks = (float)$marks;
        if ($marks < 0 || $marks > $total) { echo json_encode(['success'=>false,'message'=>"Marks must be 0–{$total}."]); exit; }

        $db->prepare('INSERT INTO test_scores (student_user_id, teacher_id, test_name, marks_obtained, total_marks, test_date) VALUES (?,?,?,?,?,?)')
           ->execute([$uid, $user['id'], $name, $marks, $total, $date]);
    ...
    <ul class="nav" style="gap:4px">
      ...
      <li class="nav-item">
        <a href="teacher_students.php?class_id=<?= $cls['id'] ?><?= $search ? '&search='.urlencode($search) : '' ?>"
           ...
           style="font-size:.85rem;font-weight:600;padding:10px 16px;border-radius:8px;text-decoration:none;
                  <?= $cls['id'] === $activeClassId ? 'background:#3b82f6;color:#fff;' : 'color:#64748b;' ?>">
    ...
    <div class="d-flex align-items-center gap-3">
      ...
      <div>
        <div style="color:#94a3b8;font-size:.75rem;font-weight:600;text-transform:uppercase">
          <?= count($myClassRows) > 1 ? 'Active Class' : 'Your Class' ?>
        </div>
  ...
  <div class="table-responsive">
    <table class="table table-custom">
      ...
      <tbody>
          ...
          <td>
            <div class="d-flex align-items-center gap-2">
              <span class="badge rounded-pill bg-info text-dark"
                    id="exam-count-<?= $s['id'] ?>" style="min-width:24px">
          ...
          <td>
            <div class="d-flex align-items-center gap-2">
              <span class="badge rounded-pill bg-warning text-dark"
                    id="test-count-<?= $s['id'] ?>" style="min-width:24px">
        ...
        <tr>
          <td colspan="8" class="text-center text-muted py-4">No students in this class yet.</td>
        </tr>
        <?php endif; ?>
      </tbody>
    </table>
    ...
    <div class="modal-content" style="background:#0f172a;border:1px solid #1e3a5f;border-radius:16px">
      ...
      <div class="modal-header" style="border-bottom:1px solid #1e3a5f">
        <div>
          ...
          <div id="scoresModalStudent" style="font-size:.75rem;color:#94a3b8;margin-top:2px"></div>
        </div>
...
document.addEventListener('click', function(e) {
  ...
  document.getElementById('scoresModalTitle').textContent   = isExam ? 'Exam Scores' : 'Test Scores';
  document.getElementById('scoresModalIcon').className      = isExam ? 'bi bi-mortarboard me-2 text-info' : 'bi bi-clipboard-check me-2 text-warning';
  document.getElementById('scoresModalStudent').textContent = name;
  document.getElementById('addFormLabel').textContent       = isExam ? 'Exam Score' : 'Test Score';
  document.getElementById('scoresFooterNote').textContent   = isExam ? 'Exam scores are visible on the student dashboard.' : 'Test scores are visible on the student dashboard.';
  document.getElementById('newScoreName').placeholder       = isExam ? 'e.g. Mid Term, Final' : 'e.g. Unit Test 1, Quiz';
  ...
  ['scoreNameError','marksObtainedError'].forEach(id => document.getElementById(id).textContent = '');

  loadScores();
  bootstrap.Modal.getOrCreateInstance(document.getElementById('scoresModal')).show();
});

function loadScores() {
  document.getElementById('scoresListWrapper').innerHTML =
  ...
  fetch(window.location.pathname, {method:'POST',body:fd})
    .then(r=>r.json())
    .then(res => { if (res.success) renderScoresList(res.scores); else document.getElementById('scoresListWrapper').innerHTML=`<div class="text-danger small">${esc(res.message)}</div>`; })
    .catch(() => { document.getElementById('scoresListWrapper').innerHTML='<div class="text-danger small">Failed to load scores.</div>'; });
}
...
function renderScoresList(scores) {
  ...
  if (!scores.length) {
    wrapper.innerHTML = `<div class="text-center py-3" style="color:#64748b"><i class="bi bi-inbox fs-3 d-block mb-2"></i>No ${label.toLowerCase()} scores recorded yet.</div>`;
    return;
  }
  let html = `<div class="table-responsive"><table class="table table-sm mb-0" style="font-size:.82rem">
    <thead><tr style="color:#94a3b8;border-color:#1e3a5f">
      <th>${label} Name</th><th>Marks</th><th style="min-width:110px">Progress</th><th>Grade</th><th>Date</th><th></th>
    </tr></thead><tbody id="scoresTableBody">`;
  scores.forEach(s => {
    const pct = s.total_marks > 0 ? (s.marks_obtained / s.total_marks * 100) : 0;
    const gi  = gradeInfo(pct);
    html += `<tr style="border-color:#1e3a5f" id="score-row-${s.id}">
      <td class="text-white align-middle fw-600">${esc(s.score_name)}</td>
      <td class="align-middle"><span class="badge bg-primary">${parseFloat(s.marks_obtained).toFixed(1)}/${parseFloat(s.total_marks).toFixed(0)}</span></td>
      <td class="align-middle">
        <div class="d-flex align-items-center gap-2">
          <div class="progress flex-grow-1" style="height:5px;background:#1e293b;border-radius:99px">
            <div class="progress-bar" style="width:${Math.min(100,Math.round(pct))}%;background:${gi.color};border-radius:99px"></div>
          </div>
          <span style="font-size:.7rem;color:#94a3b8;min-width:34px">${pct.toFixed(0)}%</span>
        </div>
      </td>
      <td class="align-middle"><span class="badge rounded-pill px-2" style="background:${gi.color}22;color:${gi.color};border:1px solid ${gi.color}55;font-size:.7rem">${gi.label}</span></td>
      <td class="align-middle text-muted small">${esc(s.score_date||'—')}</td>
      <td class="align-middle text-end">
        <button class="btn btn-sm btn-outline-danger py-0 px-2" style="font-size:.72rem" onclick="deleteScore(${s.id})" title="Delete"><i class="bi bi-trash"></i></button>
      </td>
    </tr>`;
  });
...
function submitAddScore() {
  ...
  let valid = true;
  if (!name)  { document.getElementById('scoreNameError').textContent = (scoreState.type==='exam'?'Exam':'Test')+' name is required.'; valid=false; }
  if (!marks || isNaN(parseFloat(marks))) { document.getElementById('marksObtainedError').textContent='Marks are required.'; valid=false; }
  else if (parseFloat(marks)<0||parseFloat(marks)>total) { document.getElementById('marksObtainedError').textContent=`Must be 0–${total}.`; valid=false; }
  if (!valid) return;
  ...
  fd.append('student_user_id', scoreState.uid);
  fd.append('score_name', name); fd.append('marks_obtained', marks);
  fd.append('total_marks', total); fd.append('score_date', date);
  fetch(window.location.pathname, {method:'POST',body:fd})
    .then(r=>r.json()).then(res=>{
      btn.disabled=false; btn.innerHTML='<i class="bi bi-plus-lg me-1"></i>Add';
      if (res.success) {
    ...
    }).catch(()=>{ btn.disabled=false; btn.innerHTML='<i class="bi bi-plus-lg me-1"></i>Add'; document.getElementById('scoreNameError').textContent='Network error.'; });
}
  ...
  fetch(window.location.pathname, {method:'POST',body:fd})
    .then(r=>r.json()).then(res=>{
      if (res.success) {
        ...
        const cid = scoreState.type==='exam'?`exam-count-${scoreState.uid}`:`test-count-${scoreState.uid}`;
        const cel = document.getElementById(cid);
        if (cel) cel.textContent = Math.max(0, parseInt(cel.textContent||1)-1);
        showGlobalAlert('success', res.message);
      } else { showGlobalAlert('danger', res.message); }
    }).catch(()=>showGlobalAlert('danger','Network error.'));
}
</script>
